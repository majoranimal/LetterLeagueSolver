...
def findPossibleWords(gridLocation, gridLocationsUsed, currentWord, possibleWords):
    ...
    if currentWord+currentGrid.get(gridLocation) in possibleWords:
        correctWords.append({"word":currentWord+currentGrid.get(gridLocation), "gridLocations":gridLocationsUsed+[gridLocation]})
    
    newPossibleWords = []
    for word in possibleWords:
        if word.startswith(currentWord+currentGrid.get(gridLocation)) and word != currentWord+currentGrid.get(gridLocation):
            newPossibleWords.append(word)
    
    if newPossibleWords != []:
        for letter in possibleLetters:
            if gridLocation+letter in possibleTiles and gridLocation+letter not in gridLocationsUsed:
                correctWords = correctWords+(findPossibleWords(gridLocation+letter, gridLocationsUsed+[gridLocation], currentWord+currentGrid.get(gridLocation), newPossibleWords))
...
def calculateWord():
    ...
    if doublesEntry.get("DL").get() == "":
        ...
        doubleLetter = Point(int(doublesEntry.get("DL").get().split(",")[0]), int(doublesEntry.get("DL").get().split(",")[1]))
    
    if doublesEntry.get("DW").get() == "":
        doubleWord = Point(0,0)
    else:
        doubleWord = Point(int(doublesEntry.get("DW").get().split(",")[0]), int(doublesEntry.get("DW").get().split(",")[1]))
    
    for row in range(1, rowSize+1):
        for col in range(1, columnSize+1):
            validWords = validWords + findPossibleWords(Point(row, col), [], "", copy.copy(wordlist))
    ...
    global result
    result.set(str(validWords[0].get("word")) + "(" + str(validWords[0].get("worth")) + "): " + str(validWords[0].get("gridLocations")))
...
def nextWord():
    ...
    try:
        ...
        result.set(str(newWord.get("word")) + "(" + str(newWord.get("worth")) + "): " + str(newWord.get("gridLocations")))
    except:
        print("Could not identify the next word")

# Adds a button that runs calculateWord
btn = Button(frame, width=2, height=2, text="Calculate", command=lambda: calculateWord())
